...
trin_set = train_gen.flow_from_directory('Data_covid-19/train', target_size=(256, 256), batch_size=16,
                                         subset='training')
val_set = train_gen.flow_from_directory('Data_covid-19/train', target_size=(256, 256), batch_size=16,
                                        subset='validation')

########################## Fit ##########################
history = model.fit(trin_set,
                    steps_per_epoch=30,  # how num of image will be train
                    epochs=80,  # num of cycle for per image & to make more accuracy (high epochs)--> epochs=15 or more
...
#model.save('Covid-19ResNet101.h5')

'''
 - For Training data set:
    accuracy: 98.44% 
    Loss: 0.09483683109283447

 - For validation data set:
    accuracy: 90.62% 
    Loss: 0.2372460514307022
'''
